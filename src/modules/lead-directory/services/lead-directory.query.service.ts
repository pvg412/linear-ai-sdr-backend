...
export class LeadDirectoryQueryService {
  ...
  async listAllFlat(
    ...
    const unassignedCount = withUnassigned
      ? await this.repo.countUnassignedLeads({ ownerId })
      : 0;
    ...
    const items = withUnassigned
      ? [makeUnassignedDirectory(ownerId, unassignedCount), ...sorted]
      : sorted;
