 ...
 */
export function splitSearchLeadsBulkFilter(filter: SearchLeadsFilter): {
  filters: SearchLeadsFilter[];
  bulkKey?: string;
} {
  ...
  const bulkEntry = entries.find(([k, v]) => {
    ...
    return (
      isStringArray(v) && v.length > SEARCHLEADS_LIMITS.maxBulkItemsPerRequest
    );
  });
  ...
  for (
    let i = 0;
    i < bulkValue.length;
    i += SEARCHLEADS_LIMITS.maxBulkItemsPerRequest
  ) {
    const chunk = bulkValue.slice(
      i,
      i + SEARCHLEADS_LIMITS.maxBulkItemsPerRequest,
    );
    out.push({
