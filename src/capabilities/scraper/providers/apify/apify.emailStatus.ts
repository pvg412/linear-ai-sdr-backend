...
function parseApifyEmailEntry(v: unknown): ApifyEmailEntry | null {
	...
	return { email, status, deliverable, catchAllDomain, validEmailServer, qualityScore };
}
...
function normalizeApifyEmailEntry(entry: ApifyEmailEntry): EmailStatus {
	...
	if (status) {
		if (status.includes("catch") || status.includes("accept")) return "CATCH_ALL";
		if (status.includes("risk")) return "RISKY";
		if (status.includes("invalid") || status.includes("undeliver")) return "UNDELIVERABLE";
		if (status.includes("valid") || status.includes("deliver")) return "DELIVERABLE";
