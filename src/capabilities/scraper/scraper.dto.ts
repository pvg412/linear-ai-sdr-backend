...
const AutoSegLevelSchema = z.enum(["default", "country", "state", "seniority_level", "industry"]);
const ApifyDedupModeSchema = z.enum(["off", "insert_ids", "insert_profiles", "read_only"]);
const ProfileScraperModeSchema = z.enum(["Short", "Full", "Full + email search"]);

export const ApifyScraperQuerySchema = z
  .looseObject({
    ...
    autoQuerySegmentationTargetCountries: z.array(z.string().trim().min(2).max(2)).optional(),
    ...
    postFilteringMongoDbAggregation: z.array(z.record(z.string(), z.unknown())).optional(),
  });
...
export type ScraperCityScraperQuery = z.infer<typeof ScraperCityScrapeQuerySchema>;
